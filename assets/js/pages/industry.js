...
const table_columns = [
            ...
            return `
                <div class="cus-toggle">
                    ...
                    <label class="cus-switch">
                        <input type="checkbox" ${data == 1 ? "checked" : ""} data-pid="${row["web_industry_id"]}" class="industry_status">
    ...
    {
        data: "web_industry_id",
        width: "200px",
        className: "text-center my-btn position-relative",
        title: "Action",
        render: function (data, type, row) {
            return `
                <button data-pid="${row["web_industry_id"]}" class="waves-effect waves-light btn btn-primary-light btn-sm edit-industry" type="button"><i class="fi fi-sr-attribution-pencil"></i> Edit</button>
                <button data-pid="${row["web_industry_id"]}" class="waves-effect waves-light btn btn-danger-light btn-sm del-industry" type="button"><i class="fi fi-sr-trash"></i> Delete</button>
            `;
        }
    }
];
...
$(document).on("click", ".edit-industry", function () {
    ...
    $(".brand-form input[name=web_industry_id]").val(pid);

    // Fetch the business data and last_count
    const result = common.ajax_fech("getAboutOurIndustry");
    ...
    if (pid > 0) {
        const row = Array.isArray(result.data) ? result.data.find(r => r.web_industry_id == pid) : null;
        if(row) {
            ...
            $(".brand-form input[name=web_industry_id]").val(row["web_industry_id"]);
...
$(".brand-form").on("submit", function (event) {
    ...
    common
        .ajax_save_file("saveAboutOurIndustry", formData)
...
// Delete handler
$(document).on("click", ".del-industry", function () {
    ...
    swal.fire({
        ...
        if (result.isConfirmed) {
            common
                .ajax_save("saveAboutOurIndustry", update_data)
                .then(function (response) {
                    if (response.code == 200) {
...
// Status update
$(document).on("change", ".industry_status", function () {
    const pid = $(this).data("pid");
    const update_data = {
        web_industry_id: pid,
        for: "status",
