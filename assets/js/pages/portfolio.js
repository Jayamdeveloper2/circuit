$(function () {
    ...
    $(document).on('click', '.edit-portfolio', function () {
        ...
        if (id != -1) {
            $.post('api/getPortfolioDomain', { web_portfolio_domain_id: id }, function (res) {
                if (res.status == 'success' && res.data.length > 0) {
                    let row = res.data[0];
                    form.find('[name="web_portfolio_domain_id"]').val(row.web_portfolio_domain_id);
                    ...
                    modal.find('#web_image_prev').removeClass('d-none').attr('src', row.web_image);
                    modal.modal('show');
                }
            });
        } else {
    ...
    $(document).on('click', '.delete-portfolio', function () {
        ...
        if (confirm('Are you sure you want to delete this domain tile?')) {
            $.post('api/savePortfolioDomain', { web_portfolio_domain_id: id, for: 'delete' }, function (res) {
                if (res.status == 'success') {
                    portfolioTable.ajax.reload();
                }
            });
    ...
    $(document).on('change', '.status-toggle', function () {
        ...
        $.post('api/savePortfolioDomain', { web_portfolio_domain_id: id, is_active: status, for: 'status' }, function (res) {
            if (res.status != 'success') {
